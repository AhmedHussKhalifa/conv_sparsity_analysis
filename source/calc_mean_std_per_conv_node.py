    ...
    for  nid, n in enumerate(graph_def.node):
        try:
            ...
            if ('Conv2D' in n.name or 'convolution' in n.name) and '_bn_' not in n.name and 'Logits' not in n.name and 'logits' not in n.name:
                ...
                # No need
                # write_conv_filter(sess.run(conv_tensor_params), len(k_tensor_list))
...
def write_mean_std_per_conv_node_for_each_image(input_to_conv_node, imgID, ic):
    
    N, H, W, C = input_to_conv_node.shape

    # Define the input spatial dimension
    input_spatial_dim = H*W

    # Get the mean across C, and std across C

    # Remve the n 
    input_to_conv_node = input_to_conv_node[0, :, :, :]

    # Some inputs are entirely zero.
    # Get the total number of non zero elements in each C
    denisty_channel_list = np.zeros(C)
    nze_channel_list = np.zeros(C)
    for c in range(C):
        denisty_channel_list[c] = np.count_nonzero(input_to_conv_node[:, :, c])/input_spatial_dim
        nze_channel_list[c]     = np.count_nonzero(input_to_conv_node[:, :, c])
        #print(c, denisty_channel_list[c])

    sum_density_conv_node  = np.sum(nze_channel_list)
    mean_density_conv_node = np.mean(denisty_channel_list)
    std_density_conv_node  = np.std(denisty_channel_list)


    mean_filname = FLAGS.gen_dir +  (FLAGS.model_name +  "_Conv_%d.stat") % (ic)
    with open(mean_filname, 'a') as f:
        f.write("%f\t%f\t%d\n" % (mean_density_conv_node, std_density_conv_node, sum_density_conv_node))
        # f.write("%f\t%f\n" % (mean_density_conv_node, std_density_conv_node))

    # print(mean_density_conv_node, std_density_conv_node)
    
   

def calc_mean_std_per_conv_node_for_each_image(sess, all_layers, first_input_tensor, image_data, imgID):

    # Node 87 in IV3 is a golden node for us... 
    # print('Generate ImgID %d ' % imgID)
    for ilayer, layer in enumerate(all_layers): # 52, 18
        ic = ilayer
        c = layer.output_tensor_name
        ...
        write_mean_std_per_conv_node_for_each_image(input_to_feature_map, imgID, ic)



def write_layer_dims(all_layers, imgID):
    ...
    for ilayer, layer in enumerate(all_layers): # 52, 18
        ...
        mean_filname = FLAGS.gen_dir +  (FLAGS.model_name +  "_Conv_%d.stat") % (ic)
        with open(mean_filname, 'w') as f:
            f.write("%d\t%d\t%d\t%d\t%d\n" % (layer.Ih, layer.Iw, layer.Kh, layer.Kw, layer.Ic))



def write_conv_filter(filter_values, ic):

    # Write the input first:
    filter_shape = filter_values.shape
    filter_values_vector = np.reshape(filter_values, filter_shape[0]*filter_shape[1]*filter_shape[2]*filter_shape[3])


    filter_filname = FLAGS.gen_dir +  (FLAGS.model_name +  "_Conv_%d.weight") % (ic)
    with open(filter_filname, 'w') as f:
        for item in filter_shape:
            f.write("%d\n" % item)

        for item in filter_values_vector:
            f.write("%f\n" % item)
...
def readAndPredictOptimizedImageByImage():
    ...
    for imgID in range(FLAGS.START, FLAGS.END):
        ...
        # Calculate the information of the architecture
        if actual_idx == FLAGS.START:
            all_layers_info, first_input_tensor       = get_DNN_info_general(sess, first_jpeg_image)

            # Write the layer dimensions:
            write_layer_dims(all_layers_info, original_img_ID)


        # generate_the_dataset(sess, all_layers_info, first_input_tensor, image_data, original_img_ID)      
        calc_mean_std_per_conv_node_for_each_image(sess, all_layers_info, first_input_tensor, image_data, original_img_ID)
...
def main(_):
    ...
    ensure_dir_exists(FLAGS.gen_dir)
